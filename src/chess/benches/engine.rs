...
use chess::model::game_state::{GameState, GameStatus};
use divan::Bencher;
...
fn cache_for_moves(bencher: Bencher) {
  ...
  bencher.bench_local(|| {
           if false == cache.has_move_list(&game_state.board) {
             ...
             assert_eq!(move_list, cache.get_move_list(&game_state.board).unwrap().to_vec());
           }
         });
}
...
fn cache_for_game_status(bencher: Bencher) {
  ...
  bencher.bench_local(|| {
           ...
           if eval_cache.depth == 0 {
             cache.set_eval(&game_state.board, EvaluationCache { game_status,
                                                                 eval: f32::NAN,
                                                                 depth: 1 });
           } else {
...
fn cache_for_evals(bencher: Bencher) {
  ...
  assert!(!eval.is_nan(), "Evaluate board returned NaN eval for : {}", game_state.to_fen());
