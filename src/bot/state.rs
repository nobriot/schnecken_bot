...
impl BotState {
  ...
  pub async fn new(api_token: &str) -> Self {
    let api = LichessApi::new(api_token);
  ...
  ///
  async fn on_incoming_challenge(&self, challenge: lichess::types::Challenge) {
    ...
    match json_value["type"].as_str().unwrap() {
      ...
      "challenge" => {
        ...
        if let Err(error) = result {
          ...
          let bot_clone = self.clone();
          tokio::spawn(async move { bot_clone.on_incoming_challenge(result.unwrap()).await });
        }
      },
      "challengeCanceled" => {
